...
def create_random_file():
    """Create 10GB file with random content"""
    print("Creating 10GB random file...")
    subprocess.run(['dd', 'if=/dev/urandom', 'of=/tmp/test_file.dat', 'bs=1M', 'count=10240'], check=True)
...
def record_to_csv(snapshot_num, elapsed_time):
    """Record results to CSV"""
    file_exists = os.path.exists('snapshot_results.csv')
    
    with open('snapshot_results.csv', 'a', newline='') as csvfile:
...
def main():
    try:
        ...
        record_to_csv(snapshot_num, elapsed_time)
        
        # Step 5: Copy snapshot and create AMI
        ami_id = copy_snapshot_and_create_ami(snapshot_id, current_region)
        
        print(f"Process completed successfully!")
        print(f"Snapshot: {snapshot_id}")
        print(f"AMI: {ami_id}")
        print(f"Results saved to snapshot_results.csv")
